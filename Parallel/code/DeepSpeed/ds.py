...
def main():
    ...
    # model.to(device)  # deepspeed will do this
    ...
    train_dataloader = DataLoader(train_dataset,batch_size=args.micro_train_bs,collate_fn=default_data_collator,sampler=DistributedSampler(train_dataset))
    ...
    model_engine, _, __, ___ = deepspeed.initialize(
                                                model = model,
    ...
    for epoch in range(args.n_epoch):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            outputs = model_engine(input_ids=batch['input_ids'].to(device),
            ...
            if model_engine.is_gradient_accumulation_boundary():
                ...
                if completed_step>0 and completed_step % args.checkpoint_step == 0 and not args.streaming:
                    #Important: all processes must call this method and not just the process with rank 0. It is because each process needs to save its master weights and scheduler+optimizer states.
                    model_engine.save_checkpoint(args.output_dir,tag='{}step_ckpt'.format(completed_step))
