...
def main():
    ...
    model.to(device)  # must first 
    #model  = DataParallel(model,device_ids=list(range(torch.cuda.device_count())))
    size_wrap_policy = functools.partial(
        size_based_auto_wrap_policy, min_num_params=20000
    )
    block_wrap_policy = functools.partial(
    ...
    model = FSDP(model,
                auto_wrap_policy=block_wrap_policy,
                cpu_offload=CPUOffload(offload_params=True),
                mixed_precision=bf16_policy if bf16_ready else fp16_policy,
                sharding_strategy=ShardingStrategy.FULL_SHARD, ShardingStrategy.SHARD_GRAD_OP,
                backward_prefetch = BackwardPrefetch.BACKWARD_PRE,)#, device_ids=[args.local_rank], output_device=args.local_rank)
    #model = FSDP(model)
    ...
    # sampler must be used exclusively with shuffle
    train_dataloader = DataLoader(train_dataset,batch_size=args.micro_train_bs,shuffle=False,collate_fn=default_data_collator,sampler=DistributedSampler(train_dataset,num_replicas=dist.get_world_size(),rank=dist.get_rank()))
    ...
    for epoch in range(args.n_epoch):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if (step + 1) % args.accum_step == 0 or (not args.streaming and step == len(train_dataloader) - 1):
                grad_norm=FSDP.clip_grad_norm_(args.clip)
                if grad_norm >= 1e2:
                ...
                if not args.streaming and completed_step >= args.max_train_step:
                    break
                if completed_step and completed_step  %  args.print_step ==0 and (args.local_rank == -1  or args.local_rank == 0):
                ...
                if completed_step>0 and completed_step % args.checkpoint_step == 0 and not args.streaming:
                    ...
                    if dist.get_rank() == 0:
                        torch.save(cpu_state_dict, os.path.join(args.output_dir,'{}step_ckpt'.format(completed_step)))
